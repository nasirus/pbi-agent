...
- Released and published v0.9.0: release PR #299 merged to master at `073dbf4`, tag/GitHub Release `v0.9.0` published, PyPI wheel/sdist verified; initial Release workflow failed on transient PyPI 503, rerun succeeded, and release notes frontmatter was stripped with `gh release edit`.
- Sandbox container names now use explicit Docker `--name pbi-agent-{normalized-workspace-folder}` for both sandbox web/run; normalization lowercases, replaces invalid chars with `-`, trims separators, and falls back to `workspace`. Focused sandbox pytest, Ruff, format check, and basedpyright passed.
