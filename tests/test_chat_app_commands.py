...
@pytest.mark.asyncio
async def test_mcp_command_is_forwarded_to_session(monkeypatch) -> None:
    monkeypatch.setattr(ChatApp, "_run_session", lambda self: None)
    app = ChatApp(settings=Settings(api_key="test-key", model="gpt-5.4-2026-03-05"))
    ...
    async with app.run_test() as pilot:
        ...
        await app._submit_user_message("/mcp")
        await pilot.pause()

        bridge.submit_input.assert_called_once_with("/mcp", image_paths=None)


@pytest.mark.asyncio
async def test_agents_command_is_forwarded_to_session(monkeypatch) -> None:
    monkeypatch.setattr(ChatApp, "_run_session", lambda self: None)
    app = ChatApp(settings=Settings(api_key="test-key", model="gpt-5.4-2026-03-05"))
    ...
    async with app.run_test() as pilot:
        ...
        await app._submit_user_message("/agents")
        await pilot.pause()

        bridge.submit_input.assert_called_once_with("/agents", image_paths=None)
