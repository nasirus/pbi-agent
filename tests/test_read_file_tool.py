...
def test_read_file_returns_image_summary_and_attachment(
    ...
    result = read_file_tool.handle(
        {"path": "chart.png"},
        ToolContext(settings=SimpleNamespace(provider="openai")),
    )
...
def test_read_file_rejects_images_for_unsupported_providers(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    result = read_file_tool.handle(
        {"path": "chart.png"},
        ToolContext(settings=SimpleNamespace(provider="xai")),
    )

    assert result == {
        "error": "provider 'xai' does not support image inputs in this build"
    }


def test_read_file_rejects_unsupported_image_formats(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
