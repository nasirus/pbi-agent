...
Evaluate whether completed work is production-shippable. Base answer on current conversation, workspace state, implemented changes, review history, and validation evidence. Do not modify files unless user explicitly asks for fixes.
...
Answer core question directly: whether work is ready to ship in production.

Calibrate confidence as percentage/range:
...
- 50-69%: not comfortable shipping without more checks/fixes.
...
- Full validation has not run after latest fix.
- Review found repeated regressions in same area.
- Unresolved TODOs, failing checks, or unexplained dirty files exist.
...
Adapt first line when confidence lower:

- `No — I would not ship this yet.`
- `Not yet — I’d want more validation first.`
- `Yes — I’d ship this after the final gate passes.`

Keep response short and evidence-based. Do not overstate certainty. Mention exact files/checks only when materially affecting confidence.
